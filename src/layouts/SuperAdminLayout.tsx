    ...
    <div className="min-h-screen bg-background flex">
      ...
      <aside className="hidden lg:flex flex-col w-[280px] bg-sidebar border-r border-sidebar-border fixed left-0 top-0 bottom-0 z-40">
        ...
        <div className="p-4 border-t border-sidebar-border">
          ...
          <Button variant="ghost" className="w-full justify-start text-sidebar-foreground hover:bg-sidebar-accent" onClick={handleSignOut}>
            <LogOut size={18} /><span className="ml-2">Sign Out</span>
          </Button>
        </div>
      </aside>
      ...
      {mobileMenuOpen && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          className="lg:hidden fixed inset-0 bg-background/80 backdrop-blur-sm z-30" onClick={() => setMobileMenuOpen(false)} />
      )}
